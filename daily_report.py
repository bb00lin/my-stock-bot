import os
import yfinance as yf
import pandas as pd
import requests
import datetime
import time

# ==========================================
# 1. 環境設定
# ==========================================
LINE_ACCESS_TOKEN = os.getenv("LINE_ACCESS_TOKEN")
LINE_USER_ID = os.getenv("LINE_USER_ID")
WATCH_LIST = ["2330", "2317", "2882", "2886", "6223", "8069", "6770", "1101"]

def send_line_message(message):
    if not LINE_ACCESS_TOKEN or not LINE_USER_ID: return
    url = "https://api.line.me/v2/bot/message/push"
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {LINE_ACCESS_TOKEN}"}
    payload = {"to": LINE_USER_ID, "messages": [{"type": "text", "text": message}]}
    requests.post(url, headers=headers, json=payload)

def get_tw_stock(sid):
    clean_id = str(sid).strip().upper()
    for suffix in [".TW", ".TWO"]:
        target = f"{clean_id}{suffix}"
        stock = yf.Ticker(target)
        if not stock.history(period="1d").empty:
            return stock, target
    return None, None

# ==========================================
# 2. 完整指標抓取邏輯
# ==========================================
def fetch_metrics(sid):
    stock, full_id = get_tw_stock(sid)
    if not stock: return None
    
    try:
        df = stock.history(period="7mo")
        info = stock.info
        curr = df['Close'].iloc[-1]
        prev = df['Close'].iloc[-2]
        
        # 中文名稱映射
        raw_name = info.get('shortName', sid).upper()
        name_map = {
            "TAIW": "台積電", "HON HAI": "鴻海", "CATHAY": "國泰金", 
            "MEGA": "兆豐金", "TCC": "台泥", "POWERCHIP": "力積電",
            "MPI": "旺矽", "E INK": "元太"
        }
        name = sid
        for k, v in name_map.items():
            if k in raw_name:
                name = v
                break

        # 1. 漲跌幅指標
        d1 = ((curr / prev) - 1) * 100
        m1 = ((curr / df['Close'].iloc[-22]) - 1) * 100
        m6 = ((curr / df['Close'].iloc[0]) - 1) * 100
        
        # 2. 基本面與估值指標
        margin = (info.get('profitMargins', 0) or 0) * 100
        pe = info.get('trailingPE', 0) or 0
        yield_val = (info.get('dividendYield', 0) or 0) * 100
        
        # 3. 狀態判定
        is_gem = margin > 0 and yield_val >= 5.0 and m6 > 0
        status = "💎鑽石" if is_gem else ("🔥強勢" if m6 > 0 else "☁️盤整")
        if margin < 0: status = "⚠️虧損"

        market = "市" if ".TW" in full_id and ".TWO" not in full_id else "櫃"

        return {
            "標的": f"{name}({sid}{market})",
            "價格": round(curr, 1),
            "1D": f"{d1:+.1f}%",
            "1M": f"{m1:+.1f}%",
            "PE": f"{pe:.1f}" if pe else "N/A",
            "淨利": f"{margin:.1f}%",
            "狀態": status
        }
    except Exception as e:
        return None

def main():
    results = []
    for sid in WATCH_LIST:
        data = fetch_metrics(sid)
        if data: results.append(data)
        time.sleep(1)
    
    if not results: return
    
    now = datetime.datetime.now().strftime("%Y/%m/%d")
    msg = f"📊 【{now} 台股多指標追蹤】\n"
    msg += "━━━━━━━━━━━━━━\n"
    # 增加表頭寬度
    msg += "名稱(代號) | 現價 | 1D | 1M | PE | 淨利 | 狀態\n"
    
    for r in results:
        # 將所有缺失的指標全部組合回一行
        msg += f"{r['標的']} | {r['價格']} | {r['1D']} | {r['1M']} | {r['PE']} | {r['淨利']} | {r['狀態']}\n"
    
    msg += "━━━━━━━━━━━━━━\n"
    msg += "註：💎=獲利+高息+多頭"
    
    send_line_message(msg)

if __name__ == "__main__":
    main()
