...
def generate_and_save_summary(data_list, report_time_str):
    ...
    6. ✨【★ 明日券商 APP 智慧單下單精確設定】：
       深度交叉比對第二章潛力股與第四章黃金公式清單。挑選出 1~2 檔執行獵殺：
       (A) ✨ 特選：低位階尚未起飛股 (MA5 貼近 MA20，日漲跌 0%~3%，量縮)
       (B) ⚡ 衝刺：中位階主升起飛股 (MA5>MA10>MA20>MA60，日漲跌 3%~6%，溫和出量)
       (C) 🚀 獵殺：高位階極速飆高股 (乖離大，日漲跌 > 6% 或鎖漲停)
       
       ⚠️【最高警告】：絕對禁止使用「為什麼選它」這種空泛段落！你必須強制套用下方「進場邏輯深度解析」的 1~3 點清單格式，並且必須把大括號內的數據換成真實數字！若未寫出具體 MA 價格或張數，報告無效！
    ...
    ### ★ 明日券商 APP 智慧單下單精確設定
    🎯 獵殺目標一：[股票名稱] (代號) - [套用 A, B, C 等級名稱]
    - 📊 進場邏輯深度解析 (黃金公式大數據拆解)：
      1. 【大趨勢保護】：現價 ([填寫實際現價]元) 位於 MA20 月線 ([填寫實際MA20]元) 與 MA60 季線 ([填寫實際MA60]元) 之上，呈現多頭排列。
      2. 【洗盤與量能表態】：今日成交量為 [填寫實際今日張數]張，對比 5 日均量 [填寫實際5日均量張數]張，顯示 [簡述籌碼意義]。
      3. 【位階安全防禦】：今日漲跌幅為 [填寫實際日漲跌幅]%，股價回測至 MA5 ([填寫實際MA5]元) 附近。
    - 精確進場區間：AI 總監提示「回測 MA5 ([填寫實際MA5]元) 附近進場」
    - APP 實戰設定步驟：
      1. 開啟手機券商 APP，切換到「條件單」或「智慧單」功能（選擇長效期智慧單）。
      2. 觸發條件設定：當股價小於或等於 [實際MA5 + 0.1元] 時。
      3. 下單動作設定：以「限價 [實際MA5元]」買入 1 張。
      4. 終極安全帶（停損設定）：同時設定一筆智慧停損單，條件為「當股價收盤跌破 MA20: [實際MA20元]」時，立刻以市價全數砍出。
    """
