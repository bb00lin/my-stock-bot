...
def fetch_pro_metrics(sid):
    ...
    try:
        ...
        # A. 成交金額計算 (億)
        amount_in_hundred_million = (curr_vol * curr_p) / 100_000_000
        if amount_in_hundred_million < MIN_AMOUNT_HUNDRED_MILLION: return None
        ...
        # E. --- 進化版評分邏輯 (12分制) ---
        score = 0
        if this_q_m > 0: score += 2        # 獲利加分
        if m6 > 0: score += 3              # 長線動能
        if "📈" in m_trend: score += 2     # 成長動能
        if dividend_yield > 3.5: score += 2   # 殖利率
        if 40 < curr_rsi < 70: score += 1     # 位階適中
        # --- 新增加分項 ---
        if amount_in_hundred_million > 10: score += 1 # 金流雄厚 (+1)
        if vol_ratio > 2.0: score += 1                # 異常大量 (+1)
...
def main():
    ...
    for r in results:
        gem = "💎 " if r['score'] >= 9 else "" # 分數上限提升，鑽石調至 9 分
        msg += f"━━━━━━━━━━━━━━\n"
